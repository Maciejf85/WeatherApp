...
Citys.forEach(x => {
  x.cities.forEach(item => {
    autoComplete.push(
      new CityList(item.text_simple, item.text, item.lat, item.lon)
    );
  });
});
...
function displayOnPage(currentNewElement) {
  ...
  if (
    $sourceData[0].country === "Poland" &&
    $sourceData[0].pm1 !== undefined &&
    $sourceData[0].pm25 !== undefined &&
    $sourceData[0].pm10 !== undefined
  ) {
    var $airQuality = $("<div>", {
      class: "airQuality"
    });
  ...
  $color.each((index, item) =>
    $(item).css("background", $sourceData[0].airColor)
  );
